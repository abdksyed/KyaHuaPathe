...
async def handle_input_media(message: Message):
    ...
    mime_type = attachment.mime_type if hasattr(attachment, "mime_type") else "image/jpeg"
    if mime_type not in SUPPORTED_MIME_TYPES:
        return None
    file_obj = await attachment.get_file()
    # We don't use file_name = attachment.file_name
    # because PhotoSize also doesn't have file_name
    # so general way to get file name will be via file_path
